...
export default function OrderButton() {
  ...
  return (
    <>
      <button 
        className={`order ${isAnimating ? 'is-animating' : ''}`}
        onClick={handleClick}
      >
        {/* Text elements remain as spans */}
        ...
        {/* Restored back to divs so the CSS geometry works perfectly */}
...
const styles = `
  ...
  /* This now correctly ONLY targets the text, not the truck! */
  .order span {
    --o: 1;
    position: absolute;
    inset: 0;
  ...
  .order.is-animating .default {
    --o: 0;
    transition-delay: 0s;
  }
  .order.is-animating .success {
    --offset: 0;
    --o: 1;
    transition-delay: 7s;
  }
  .order.is-animating .success svg {
    transition-delay: 7.3s;
  }
  .order.is-animating .truck {
    animation: truck 10s ease forwards;
  }
  .order.is-animating .truck:before {
    animation: door1 2.4s ease forwards 0.3s;
  }
  .order.is-animating .truck:after {
    animation: door2 2.4s ease forwards 0.6s;
  }
  .order.is-animating .truck .light:before, .order.is-animating .truck .light:after {
    animation: light 10s ease forwards;
  }
  .order.is-animating .box {
    animation: box 10s ease forwards;
  }
  .order.is-animating .lines {
