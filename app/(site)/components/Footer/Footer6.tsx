...
export default function Footer6() {
  return (
    <div className="@container w-full overflow-hidden rounded-[24px] border border-[var(--color-border-default)] bg-[var(--color-bg)] p-2 shadow-[0_24px_80px_color-mix(in_srgb,var(--color-bg)_40%,transparent)] sm:rounded-[28px] sm:p-3">
      <div className="overflow-hidden rounded-[18px] border border-[var(--color-border-default)] bg-[color-mix(in_srgb,var(--color-bg)_88%,black)] text-[var(--color-fg)] shadow-2xl sm:rounded-[22px]">
        <div className="flex h-9 items-center gap-2 border-b border-[var(--color-border-default)] px-4" aria-label="Browser window frame">
          ...
          <span className="ml-3 hidden min-w-0 flex-1 truncate rounded-md border border-[var(--color-border-default)] px-3 py-1 text-[9px] text-[var(--color-fg-muted)] sm:block">auraui.dev / build with intention</span>
          <span className="ml-auto text-[9px] uppercase tracking-[0.16em] text-[var(--color-fg-muted)]">Aura UI</span>
        </div>

        <footer className="px-5 pb-5 pt-7 sm:px-7 sm:pb-7 sm:pt-9 lg:px-10 lg:pb-9">
          <div className="grid min-w-0 grid-cols-1 gap-x-7 gap-y-9 sm:grid-cols-2 @min-[900px]:grid-cols-[1.35fr_repeat(4,minmax(0,1fr))_1.45fr] @min-[900px]:gap-x-8 @min-[900px]:gap-y-0">
            <div className="min-w-0 sm:col-span-2 @min-[900px]:col-span-1">
              ...
              <p className="mt-5 max-w-[230px] text-[12px] leading-relaxed text-[var(--color-fg-muted)]">A considered toolkit for building digital experiences with clarity, character, and care.</p>
              <a href="/components" className="group mt-5 inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-accent-primary)]">Explore the library <ArrowUpRight className="h-3 w-3 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></a>
            </div>
            ...
            <div className="min-w-0 border-t border-[var(--color-border-default)] pt-6 sm:col-span-2 @min-[900px]:col-span-1 @min-[900px]:border-l @min-[900px]:border-t-0 @min-[900px]:pl-7 @min-[900px]:pt-0">
              ...
              <form action="mailto:hello@auraui.dev" method="post" encType="text/plain" className="mt-4 flex min-w-0 max-w-full items-center gap-2 rounded-lg border border-[var(--color-border-default)] p-1">
                <label htmlFor="footer6-email" className="sr-only">Email address</label>
                <input id="footer6-email" name="email" type="email" required placeholder="Your email" className="min-w-0 flex-1 bg-transparent px-2 py-1.5 text-[11px] text-[var(--color-fg)] outline-none placeholder:text-[var(--color-fg-muted)]" />
          ...
          <div className="mt-9 flex flex-col gap-3 border-t border-[var(--color-border-default)] pt-4 text-[10px] text-[var(--color-fg-muted)] sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
